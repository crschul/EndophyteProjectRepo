...
rownames(SereResultsT1) <- c("Group","Genotype","Inoculation","Geno:Inoculation", "Residuals")
...
for( m in myvars){
  print(m)
  print(as.name(m))
  linmod <- lm(Serendip_data[[m]] ~ Group_or_Date + Genotype + Condition + Genotype:Condition, data = Serendip_data)
  HPH1 <- anova(linmod)
  print(HPH1)
  SereResultsT1[[m]] <- c(HPH1[1,2],HPH1[2,2],HPH1[3,2],HPH1[4,2], HPH1[5,2])
  itlist = HPH1[,5]
...
rownames(HerbResultsT1) <- c("Group","Genotype","Inoculation","Geno:Inoculation",  "Residuals")
...
for( m in myvars){
  ...
  HPH1 <- anova(linmod)
  print(HPH1)
  HerbResultsT1[[m]] <- c(HPH1[1,2],HPH1[2,2],HPH1[3,2],HPH1[4,2], HPH1[5,2])
  itlist = HPH1[,5]
  Signif_list <- append(Signif_list, itlist)
}
...
rownames(burkResultsT1) <- c("Group","Genotype","Inoculation","Geno:Inoculation", "Residuals")
...
for( m in myvars){
  ...
  HPH1 <- anova(linmod)
  print(HPH1)
  burkResultsT1[[m]] <- c(HPH1[1,2],HPH1[2,2],HPH1[3,2],HPH1[4,2], HPH1[5,2])
  itlist = HPH1[,5]
  Signif_list <- append(Signif_list, itlist)
}
...
print(sum/count) # Without Group Genotype Variance = .55
